...
<body>
                        ...
                        <table class="table table-condensed" id="tableDatosMatricula">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td colspan="3">{{ mb_strtoupper($respuesta['docente']) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </td>
    ...
    @foreach ($respuesta['fechas'] as $index => $lista)
        <table class="table table-condensed table-areas-curriculares">
            <tr>
                ...
                <th colspan="{{$lista["ultimo_dia"]}}" style="border:1px solid black;margin:0;padding:7px;text-align:center;">{{ strtoupper($lista["mes_calendario"]) }} - {{$respuesta['yearAcademico']}}</th>
            </tr>
            <tr>
                @for ($i = 1; $i <= $lista["ultimo_dia"]; $i++)
                <th style="border:1px solid black; text-align:center;">{{$i}}</th>
                @endfor
            </tr>
            <tr>
                
                @for ($i = 0; $i < $lista["ultimo_dia"]; $i++)
                <th style="border:1px solid black; text-align:center;">{{$respuesta['dias'][($lista["dia"]+$i)%7]}}</th>
                @endfor
            </tr>
            @foreach ($lista["nombre"] as $indice => $nombre)
                <tr>
                    <th style="border:1px solid black;margin:0;padding:7px; text-align:center;">{{($indice+1)}}</th>
