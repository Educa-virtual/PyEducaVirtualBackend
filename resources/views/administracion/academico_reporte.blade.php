    ...
    <main>
        ...
        <table class="cuerpo">
            <tr>
                ...
                <th class="cabecera-table">Área Curricular</th>
                @foreach ($columna as $index => $item)
                    <th>
                        AÑO : {{$item["cYAcadNombre"]}}<br>
                        GRADO : {{$item["cGradoAbreviacion"]}}
                    </th>
                @endforeach
            </tr>
            @foreach ($fila as $index => $fl)
                <tr>
                    <td>{{$index+1}}</td>
                    <td style="text-align: left">{{$fl["cCursoNombre"]}}</td>
                    @foreach ($fl["nota"] as $notas)
